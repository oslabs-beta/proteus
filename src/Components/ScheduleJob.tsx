...
export const ScheduleJob = (props: ScheduleJobProps) => {
  ...
  return (
    <div className='home-schedule-job-container'>
      <div ref={ref} onMouseEnter={(e) => renderHover(name, time, boxNumber * boxWidth + ref.current.offsetLeft, ref.current.offsetTop)} onMouseLeave={() => renderHover()} className='home-schedule-job' style={{left: `${nudge}%`, backgroundColor: color, opacity}}>
        <div></div>
      </div>
    </div>
  )
